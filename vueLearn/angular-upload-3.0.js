	...
	angular.module('dui.component.upload', []).factory('DuiFileUploaderService', function(){
		...
		function buildThumbnailUrl(thumbnailConfig){
			...
			if(isImage(thumbnailConfig.attachName)){
				if(thumbnailConfig.domain){		//文件服务器图片
					...
					var fileServerDomain = 'http://fs.dooioo.net';
					
					if(/\.com$/.test(location.hostname || '')){
						fileServerDomain = 'http://fs.dooioo.com';
					}else if(/\.org$/.test(location.hostname || '')){
						fileServerDomain = 'http://fs.dooioo.org';
					}else{
						fileServerDomain = 'http://fs.dooioo.net';
					}
		...
		*/
		...
		return {
			...
			template: '<div>\
						<button type="button" class="btn-small btn-silver in_block js_angular_fakeupload" ng-class="{disable: cannotUpload}" ng-disabled="cannotUpload">上传</button>\
						<input type="file" class="hideit js_angular_trueUpload"></input>\
						<ul class="clearfix" ng-class="{jiaoyiAttachUploader: isJiaoyi, attachUploader: !isJiaoyi}">\
			...
			link: function(scope, el, attrs){
				...
				var uploadUrl = attrs.domain ? fileServerDomain + '/store' : attrs.url;
