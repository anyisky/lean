...
(function($){
	...
	FilesUploader.prototype = {
	    ...
	    _check: function(){       //上传之前的预处理
	    	...
	    	for(var i=0, len=this.files.length; i<len; i++){
	    		if(self.opts.extReg){
	    			if(self.opts.extReg.test(this.files[i].name) === false){
	    				 throw {
		                		code: '6',
		                		message: '系统不支持您上传的文件类型',
		                		desc: '系统不支持您上传的文件类型'
		                	};
	    			}
	    		}
	            if( this.files[i].size > this._getMaxSize() ){
	                throw {
	                		code: '5',
	                		message: '文件过大',
	                		desc: '文件太大了\n最大可上传' + Math.floor(self._getMaxSize() / (1024 * 1024)) + 'M'
	                	};
	            }
	        }
	    },
	    _buildParams: function(){
	    	...
	    	var streamName = this.opts.streamNameAilas || 'fileObject';		//跟后台交互的数据接口：后台接收文件流的名字
	    	if(this.opts.multiple === true){					//一个请求上传多个文件
		                ...
		                if (xhr.readyState == 4) {
		                	if(xhr.status == 200){
		                		...
		                		if(self.opts.errorCallback){
		                			self.opts.errorCallback(response.target.responseText);
		        		    	}else{
	...
	//进度条html
	UploadBase.progressBarHtml = '<div class="barContainer wBreak js_barContainer">\
	...
	UploadBase.prototype = {
		...
		bindUpload: function(){			//绑定上传控件
            ...
            me.uploadContainer.find('.js_fakeUpload').click(function(){
            	me.uploadContainer.find('.js_trueUpload').val('');
            ...
            me.uploadContainer.find('.js_trueUpload').change(function(){
            	...
            	$.each(this.files, function(i, currentFile){
            		...
            		me.uploadContainer.find('.js_attachList')[me.opts.direction]($attachView);
		...
		uploadDocument: function(opts){
	        ...
	        return this.each(function() {
	 	       ...
	 	       DocumentUploader.prototype.buildDefaultErrorView = function(errMsg, $attachView){
	 	    	   $attachView.find('.js_attachViewContainer').empty().append('<p class="red">'+errMsg+'</p>');
